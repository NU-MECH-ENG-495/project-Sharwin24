...
BNO055::BNO055(const std::string i2c_dev, uint8_t i2c_addr) {
  this->i2c_device = i2c_dev;
...
void BNO055::init() {
  ...
  file = open(this->i2c_device.c_str(), O_RDWR);

  if (ioctl(file, I2C_SLAVE, this->i2c_addr) < 0) {
